...
// 오리지널 객체의 일부 기능만 병경하도록 프록시 객체를 만들면 된다.
...
public class SqlSessionProxy implements SqlSession {
  ...
  public SqlSessionProxy(SqlSession original, boolean transaction) {
  ...
  // 트랜잭션에서 사용하는 경우가 아닐때만, 닫아야 한다.
  @Override
  public void close() {
    if (isInTransaction) {
      return; // 트랜잭션에 포함된 상태일 때는 닫지 자원을 해제하지 않는다.
    }
    original.close();
  }
